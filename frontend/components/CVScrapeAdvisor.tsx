...
type Props = {
  ...
  onApply: (payload: { keyword?: string; searchQuery?: string }) => void;
};

export function CVScrapeAdvisor({ scrapeSource, onApply }: Props) {
  ...
  const applyBest = () => {
    ...
    toast.success("AI suggestions applied — set your location if needed");
  };

  const showMaps = scrapeSource !== "google_search";
  const showInternet = scrapeSource === "google_search" || scrapeSource === "all";
  const chips = [
    ...(showMaps ? data?.keyword_suggestions?.slice(0, 6) ?? [] : []),
    ...(showInternet ? data?.search_queries?.slice(0, 4) ?? [] : []),
  ];
          ...
          <p className="truncate text-xs text-muted-foreground">
            {loading
              ...
              : data?.recommended_keyword || data?.recommended_search_query
                ...
                : "Based on your CV & Brain profile"}
          </p>
        </div>
        <div className="flex shrink-0 gap-1.5">
          {data ? (
            <Button type="button" size="sm" variant="outline" onClick={applyBest}>
              Apply
            </Button>
          ) : null}
          <Button
            ...
            onClick={() => (data ? setOpen((v) => !v) : loadSuggestions())}
              ...
              {showInternet
                ? data.search_queries.slice(0, 4).map((item) => (
                    <button
                      ...
                      onClick={() => onApply({ searchQuery: item })}
                    >
